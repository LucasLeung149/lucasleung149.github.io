---
title: "Supersymmetry and Supergravity"
collection: teaching
type: "Postgraduate course"
permalink: /teaching/susy-sugra-oxford
venue: "Mathematical Institute, University of Oxford"
date: 17-01-2026
location: "Oxford, United Kingdom"
---

I am currently the class tutor for the Supersymmetry and Supergravity course in the MMathPhys at Oxford for Hilary Term 2026 where I was responsible for presenting problem solutions in class. 

This is an introductory course in supersymmetry, basically following Wess and Bagger.

## A few words about the course

This course is a course on 4d $N=1$ supersymmemtry and its phenomenological consequences. There is a lot of mathematical detail on each part of the course, but the course follows the logical step of introducing:

- Supersymmetry algebra
- Supersymmetric Representations and multiplets
- Superspace formalism
- Supersymmetric actions
- Renormalisation theorems
- Supersymmetry breaking
- Super-Higgs mechanism


## The classes

The classes for this course is run by me on Thursdays and Fridays of Weeks 2,5,6,8. They are held at the Mathematical Institute in room C4 (C5 and C6 for Week 5 Thurs and Fri respectively). Details of sign-up are on the Moodle page.

Let me know if you have any problems.

## Revision class

There will be a revision class held on 15 Apr 2026, at 1400 BST. I intend to cover the 2018 past paper along with Q1 from 2021 and 2022. You may also find the following questions helpful for revision:

- Oxford MMathPhys: 2020 Q1-2, 2019 Q1, 2017 Q1-4, 2016 Q1
- Cambridge Part III: 2023 Q1, 2022 Q1-2

The zoom link for the revision class is published on Moodle.

Scribble for the class is available [here](https://lucasleung149.github.io/files/susy_revision_class_scribble.pdf).


## Notes and materials
I will publish some class notes (subject to how busy I am) after each class. They will basically include the basic discussion of what I have went through in class.

You can find the latest version [here](https://lucasleung149.github.io/files/susy_class_notes-06042026.pdf).

Last updated: 6 Apr 2026.

Note that the notes are currently only updated to half of Class 3. I intend to finish the notes at some point but since I am currently very busy I might not be able to finish this before the exam. The scribbles for class 3 and 4 can be found below --

[Class 3](https://lucasleung149.github.io/files/susy_class_3_scribble.pdf).

[Class 4](https://lucasleung149.github.io/files/susy_class_4_scribble.pdf).


Health warning: if you aren't mathematically-oriented, please don't read beyond Appendix A.


